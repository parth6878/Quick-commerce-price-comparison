...
def test_milk_dataset_with_query_relevance():
    ...
    b_df = pd.read_csv(os.path.join(DATASETS_DIR, "blinkit_data.csv"))
    z_df = pd.read_csv(os.path.join(DATASETS_DIR, "zepto_data.csv"))
    a_df = pd.read_csv(os.path.join(DATASETS_DIR, "amazon_data.csv"))
...
def test_bread_category():
    ...
    blinkit_items = [
        ...
        {"store": "Blinkit", "name": "Lays Classic Salted Potato Chips 50g", "price": 20, "quantity": "50 g"},
    ]
    zepto_items = [
    ...
    eo_deal = next(
        (
            d
            for d in matched
            if "english oven" in d["canonical_name"].lower() and "brown" in d["canonical_name"].lower()
        ),
        None,
    )
